...
> **2026-07-01 — relocated.** This plan moved from the rove repo
> (`docs/plans/replay-wasm-plan.md`) to live alongside the porcelain it
> describes: the browser replay shell now lives in this repo at
> `replay/_static/` (moved out of rove's `web/` in commit fecf07b). Paths
> below that say `web/replay/_static/...` are the rove-era location — read
> them as `replay/_static/...` here. The ENGINE-side replay/sim references
> stay in rove: `docs/architecture/replay-and-sim.md` (as-built driver) and
> `docs/plans/sim-test-framework.md` (the `rewind` CLI test framework).
